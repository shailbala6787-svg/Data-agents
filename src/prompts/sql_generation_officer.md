...
- CRITICAL: If the user asks a general question (e.g., 'total records', 'total FIRs') and there are MULTIPLE tables in the schema hint, YOU MUST query ALL of them by combining them using `UNION ALL`. Do NOT query just a single table randomly. Example: `SELECT SUM(total) FROM (SELECT total FROM rnd_1 UNION ALL SELECT total FROM rnd_2)`.
